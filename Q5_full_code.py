...
for i in range(num_mentor):
    ...
    pq[mentor.at[i, "gid"]] = PriorityQueue()
...
def put_in(id, quota): # put `id` into a group
    bst = dist[id][cur[id]]
    if quota[bst[0]] > 0:
        pq[bst[0]].put((bst[1], id))
        quota[bst[0]]-=1
        return
    else:
        if not pq[bst[0]].empty(): min = pq[bst[0]].get()
        if min[0] < bst[1]:
            pq[bst[0]].put((bst[1], id))
            put_in(min[1], quota)
        else:
            pq[bst[0]].put((min[0], min[1]))
            if dist[id][cur[id]][2] != -1:
                cur[id] = dist[id][cur[id]][2]
                put_in(id, quota)
            else:
                print(id, "unsuccessfully")
...
def matching():
    ...
    for i in range(num_mentor):
        gid = mentor.at[i, "gid"]
        quota_female[gid] = quota_all[gid]//2 + quota_male[gid]
        while not pq[gid].empty():
            cur_res = int(pq[gid].get()[1])
            if cur_res in res[gid]: flag = False
